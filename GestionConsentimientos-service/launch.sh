...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "GestionConsentimientos-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name GestionConsentimientos-service apps/GestionConsentimientos-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app GestionConsentimientos-service:1.0-SNAPSHOT
    oc expose svc/GestionConsentimientos-service
else

	echo "Launching the application locally..."
	pattern="GestionConsentimientos-service"
	files=( $pattern )
	cd ${files[0]}
