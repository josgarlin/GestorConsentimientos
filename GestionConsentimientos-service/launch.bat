...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name GestionConsentimientos-service apps/GestionConsentimientos-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app GestionConsentimientos-service:1.0-SNAPSHOT
		call oc expose svc/GestionConsentimientos-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd GestionConsentimientos-service
		cd target
